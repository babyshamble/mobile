...
;(function(global, polyfill) {
    ...
    function text2Audio (text, readyFn) {
        ...
        // url = 'http://192.168.1.123/' + this.__options.socketpath + '&&yxjsoncallback=?&text=' + encodeURIComponent(text + '$0$100$0#0');
        // console.log(url);
        ...
        //     readyFn("http://192.168.1.123/yxsm/cache/voices/voices1/00d/00dd21802c958f86963ca3b360fce5b2.mp3");
        // }
            

        // var $script = document.createElement('SCRIPT');
        // $script.src = 'http://192.168.1.123/yxsmtest/index.php?m=ttt&a=snew&yxjn=d8x';
        // document.getElementsByTagName('HEAD')[0].appendChild($script);
        
        Speaker.jsonp('http://192.168.1.123/' + this.__options.socketpath + '&&yxjsoncallback=?&text=' + encodeURIComponent(text + '$0$100$0#0'), function(data) {
            if (Speaker.isFunction(readyFn)) {
                readyFn('http://192.168.1.123' + self.__options.voicespath + (data && data.msg));
            }
        }, function() {
